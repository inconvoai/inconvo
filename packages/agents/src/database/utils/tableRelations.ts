...
export function generateJoinGraph(
  ...
  const baseTableSchema = schema.find(
    (table) => table.name === startingTableName,
  );
...
function buildQueueEntry(
  ...
  const alias = parent
    ? `${parent.alias}.${relationLabel}`
    : `${sourceTable}.${relationLabel}`;

  const hop: JoinPathHop = {
    source: relation.sourceColumns.map((column) => `${sourceTable}.${column}`),
    target: relation.targetColumns.map(
      (column) => `${targetTableName}.${column}`,
    ),
  };
