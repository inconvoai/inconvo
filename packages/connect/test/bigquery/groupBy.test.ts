...
describe("BigQuery groupBy Operation", () => {
  ...
  test("Which products drive the highest order subtotal?", async () => {
    ...
    const expectedRows = await db
      ...
      .select((eb) => [
        sql`o.product_id`.as("orders_product_id"),
        sql`p.title`.as("products_title"),
        eb.fn.sum("o.subtotal").as("subtotal_sum"),
      ])
      .groupBy(["o.product_id", "p.title"])
      .orderBy("subtotal_sum", "desc")
      .orderBy("orders_product_id", "asc")
      .limit(10)
      .execute();

    const expected = expectedRows.map((row: any) => ({
      "orders.product_id": row.orders_product_id,
      "products.title": row.products_title,
      _sum: {
        "orders.subtotal": Number(row.subtotal_sum ?? 0),
      },
    }));
  ...
  test("supports HAVING on aggregates", async () => {
    ...
    const expected = expectedRows.map((row: any) => ({
      "orders.organisation_id": row.organisation_id,
      _count: { "orders.id": Number(row.count_id ?? 0) },
      _sum: { "orders.subtotal": Number(row.sum_subtotal ?? 0) },
    }));
